...
<script lang='ts'>
  ...
  import { completeSetup } from '$lib/modules/setup'
  export let step = 0
  ...
  async function checkNext () {
    if (step === 2) await completeSetup(SETUP_VERSION)
    goto(NEXT[step]!, { replaceState: true })
  }
</script>

<div class='px-6 mt-auto w-full lg:max-w-4xl'>
  <div class='border-x border-t w-full rounded-t-lg bg-muted p-4 gap-3 flex flex-col'>
