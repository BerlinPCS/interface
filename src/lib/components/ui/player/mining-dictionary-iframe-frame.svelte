<script lang='ts'>
  ...
  const dispatch = createEventDispatcher<{
    ...
    leave: undefined
    selection: {
      selection: Extract<MiningPopupFrameMessage, { type: 'selectionChanged' }>['selection']
      anchor: Pick<DOMRect, 'left' | 'right' | 'top' | 'bottom'>
    }
    tapoutside: undefined
    scrolled: undefined
    runtimeerror: string
  }>()
  ...
  $: if (frameReady && clearSelectionSignal !== lastClearSelectionSignal) {
    lastClearSelectionSignal = clearSelectionSignal
    post(makeMiningPopupHostMessage(nonce, { type: 'clearSelection', popupId }))
  }
  $: if (frameReady && highlightSelectionSignal !== lastHighlightSelectionSignal) {
    lastHighlightSelectionSignal = highlightSelectionSignal
  ...
  function frameMessage (event: MessageEvent) {
    ...
    } else if (message.type === 'selectionChanged' && message.popupId === popupId) {
      const frame = iframe.getBoundingClientRect()
      const rect = message.selection.rect
      dispatch('selection', {
        selection: message.selection,
        anchor: {
          left: frame.left + rect.x,
          right: frame.left + rect.x + rect.width,
          top: frame.top + rect.y,
          bottom: frame.top + rect.y + rect.height
        }
      })
    } else if (message.type === 'dismiss' && message.popupId === popupId) {
      dispatch('close')
    } else if (message.type === 'tapOutside' && message.popupId === popupId) {
      dispatch('tapoutside')
    } else if (message.type === 'scrolled' && message.popupId === popupId) {
      dispatch('scrolled')
